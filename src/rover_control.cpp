...
void velocityCallback(geometry_msgs::Twist twist)
{
	...
	std::cout<<"Speed:"<<twist.linear.x<<" "<<twist.linear.y<<" "<<twist.angular.z<<"\n";
	getRotationRates(rates,goal_vel,real_vel);
	//std::cout<<"Calculated speeds: "<<rates[0]<<" "<<rates[1]<<" "<<rates[2]<<" "<<rates[3]<<"\n";
	for(int i=0;i<4;i++){
...
void writeDeltas(serial::Serial *ser1, serial::Serial *ser2, float *rates){
		...
		//std::cout<<"write to port 1 "<<(int)rates[0]<<std::endl;
		...
		write[len]='\0';
		std::cout<<output_buffer<<std::endl;
		//std::cout<<"write to port 2 "<<(int)rates[1]<<std::endl;
		...
		write[len]='\0';
		std::cout<<output_buffer<<std::endl;
		//std::cout<<"write to port 3 "<<(int)rates[2]<<std::endl;
		...
		write[len]='\0';
		std::cout<<output_buffer<<std::endl;
		//std::cout<<"write to port 4 "<<(int)rates[3]<<std::endl;
...
int main(int argc, char** argv){
	...
	velcmd = nh.subscribe("/orbot/cmd_vel",1,velocityCallback);
...
void getRotationRates(float* rates, float* goal_vel,float* real_vel, float x_cm, float y_cm, float length, float width, float radius)
{
	...
	for(uint8_t i=0;i<4;i++){
		...
		rates[i]=(goal_vel[0])/radius*cos(b[i]-c)/sin(c);
		rates[i]-=(goal_vel[1])/radius*sin(b[i]-c)/sin(c);
		rates[i]-=(goal_vel[2])/radius*l*sin(b[i]-c+a)/sin(c);
	}
